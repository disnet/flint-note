...
app.whenReady().then(async () => {
    ...
    ipcMain.handle('send-message', async (_event, params) => {
        try {
            if (aiService) {
                // Use real AI service
    ...
    ipcMain.on('send-message-stream', async (event, params) => {
        try {
            if (aiService) {
                ...
                // Start the streaming
                await aiService.sendMessageStream(params.message, params.requestId, params.conversationId, params.model);
            }
            else {
    ...
    ipcMain.handle('create-note', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.createNote(params.type, params.identifier, params.content, params.vaultId);
    });
    ipcMain.handle('get-note', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.getNote(params.identifier, params.vaultId);
    });
    ipcMain.handle('update-note', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.updateNote(params.identifier, params.content, params.vaultId, params.metadata);
    });
    ipcMain.handle('delete-note', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.deleteNote(params.identifier, params.vaultId);
    });
    ipcMain.handle('rename-note', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.renameNote(params.identifier, params.newIdentifier, params.vaultId);
    });
    ipcMain.handle('move-note', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.moveNote(params.identifier, params.newType, params.vaultId);
    });
    // Search operations
    ipcMain.handle('search-notes', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.searchNotes(params.query, params.vaultId, params.limit);
    });
    ipcMain.handle('search-notes-advanced', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
    ...
    ipcMain.handle('create-note-type', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.createNoteType(params);
    });
    ipcMain.handle('get-note-type-info', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
    ...
    ipcMain.handle('update-note-type', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.updateNoteType(params);
    });
    ipcMain.handle('list-notes-by-type', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
    ...
    ipcMain.handle('create-vault', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.createVault(params.name, params.path, params.description);
    });
    ipcMain.handle('switch-vault', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
    ...
    // Link operations
    ipcMain.handle('get-note-links', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.getNoteLinks(params.identifier, params.vaultId);
    });
    ipcMain.handle('get-backlinks', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
        return await noteService.getBacklinks(params.identifier, params.vaultId);
    });
    ipcMain.handle('find-broken-links', async (_event, params) => {
        if (!noteService) {
            throw new Error('Note service not available');
        }
    ...
    ipcMain.handle('store-api-key', async (_event, params) => {
        if (!secureStorageService) {
            throw new Error('Secure storage service not available');
        }
        return await secureStorageService.updateApiKey(params.provider, params.key, params.orgId);
    });
    ipcMain.handle('get-api-key', async (_event, params) => {
        if (!secureStorageService) {
            throw new Error('Secure storage service not available');
        }
        return await secureStorageService.getApiKey(params.provider);
    });
    ipcMain.handle('test-api-key', async (_event, params) => {
        if (!secureStorageService) {
            throw new Error('Secure storage service not available');
        }
