...
export class ToolService {
            ...
            try {
                ...
                if (notes && notes.length > 0) {
                    ...
                    for (const note of notes) {
                        try {
                            const result = await this.noteService.createNote(note.type, note.title, note.content, vault_id || undefined, note.metadata);
                            results.push({
                                success: true,
                                noteId: result.id,
                                title: note.title,
                ...
                const result = await this.noteService.createNote(type, title, content, vault_id || undefined, metadata);
                return {
                    success: true,
    ...
    updateNoteTool = tool({
        ...
        execute: async ({ identifier, content, metadata, vault_id }) => {
            try {
                ...
                const result = await this.noteService.updateNote(identifier, content || '', vault_id || undefined, metadata);
                return {
                    success: true,
                    data: result,
    ...
    deleteNoteTool = tool({
        ...
        execute: async ({ identifier, confirm, vault_id }) => {
            try {
                ...
                const result = await this.noteService.deleteNote(identifier, vault_id || undefined);
                return {
                    success: true,
                    data: result,
    ...
    renameNoteTool = tool({
        ...
        execute: async ({ identifier, new_title, vault_id }) => {
            try {
                ...
                const result = await this.noteService.renameNote(identifier, new_title, vault_id || undefined);
                return {
    ...
    moveNoteTool = tool({
        ...
        execute: async ({ identifier, new_type, vault_id }) => {
            try {
                ...
                const result = await this.noteService.moveNote(identifier, new_type, vault_id || undefined);
                return {
                    success: true,
                    data: result,
    ...
    searchNotesTool = tool({
        ...
        execute: async ({ query, type_filter, limit, vault_id }) => {
            try {
                ...
                const results = await this.noteService.searchNotes(query, vault_id || undefined, limit);
                // Apply type filter if provided
    ...
    searchNotesAdvancedTool = tool({
        ...
        inputSchema: z.object({
            ...
            metadata_filters: z
                ...
                operator: z
                    .string()
                    .optional()
                    .describe("Comparison operator: '=', '!=', '>', '<', '>=', '<=', 'LIKE', 'IN'")
            }))
                .optional()
        ...
        execute: async ({ content_contains, type, created_within, created_before, updated_within, updated_before, limit, vault_id }) => {
            try {
                if (!this.noteService) {
                    return {
                        success: false,
    ...
    createNoteTypeTool = tool({
        ...
        execute: async ({ type_name, description, agent_instructions, metadata_schema, vault_id }) => {
            try {
                if (!this.noteService) {
                    return {
                        success: false,
