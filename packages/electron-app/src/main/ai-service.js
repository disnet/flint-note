...
export class AIService extends EventEmitter {
    ...
    constructor(gateway, noteService) {
        super();
    ...
    static async of(secureStorage, noteService = null) {
...
Use these tools to help users manage their notes effectively and answer their questions.`;
  ...
  `)
    ...
    async getSystemMessageWithCaching() {
     ...
     */
    createCachedMessageSegment(messages) {
    ...
    prepareCachedMessages(history) {
        ...
        const stableTokens = this.estimateTokens(stableMessages
    ...
    validateCacheConfig(config) {
        ...
        if (optimized.minimumCacheTokens > 4096) {
            logger.warn('Minimum cache tokens very high, consider lowering for better hit rates');
        }
     ...
     */
    recordCacheMetrics(systemCacheUsed, historyCacheUsed, conversationLength, estimatedTokensSaved, totalTokens, providerMetadata) {
    ...
    calculateModelCost(model, usage) {
        ...
        const cacheReadCost = ((usage.cacheReadInputTokens || 0) * rates.cacheRead) / 1_000_000;
        const cacheWriteCost = ((usage.cacheCreationInputTokens || 0) * rates.cacheWrite) / 1_000_000;
     ...
     */
    recordUsageAndCost(result, conversationId, modelName) {
    ...
    getCachePerformanceSnapshot() {
        ...
        // Calculate hit rates
        const totalSystemRequests = metrics.systemMessageCacheHits + metrics.systemMessageCacheMisses;
        const systemHitRate = totalSystemRequests > 0 ? metrics.systemMessageCacheHits / totalSystemRequests : 0;
        const totalHistoryRequests = metrics.historyCacheHits + metrics.historyCacheMisses;
        const historyHitRate = totalHistoryRequests > 0 ? metrics.historyCacheHits / totalHistoryRequests : 0;
        ...
        const overallEfficiency = totalCacheableRequests > 0 ? totalCacheHits / totalCacheableRequests : 0;
        // Calculate token savings rate
        const tokenSavingsRate = metrics.totalCacheableTokens > 0
        ...
        if (systemHitRate < 0.6 && this.cacheConfig.enableSystemMessageCaching) {
            recommendations.push('System message cache hit rate is low. Consider reviewing system message content or token threshold.');
        }
        if (historyHitRate < 0.4 && this.cacheConfig.enableHistoryCaching) {
            recommendations.push('History cache hit rate is low. Consider lowering minimumCacheTokens or reducing historySegmentSize.');
        }
        if (!this.cacheConfig.enableHistoryCaching && metrics.averageConversationLength > 8) {
            recommendations.push('Average conversation length suggests enabling history caching could provide significant benefits.');
        }
        if (tokenSavingsRate < 0.2 &&
            (this.cacheConfig.enableSystemMessageCaching ||
                this.cacheConfig.enableHistoryCaching)) {
            recommendations.push('Token savings rate is low. Consider optimizing cache thresholds or reviewing conversation patterns.');
        }
        if (this.cacheConfig.minimumCacheTokens > 2048 && historyHitRate < 0.3) {
            recommendations.push('Consider lowering minimumCacheTokens to increase history cache hit rate.');
        }
        return {
    ...
    optimizeCacheConfig() {
        ...
        if (this.cacheMetrics.averageConversationLength > 10 &&
        ...
        if (performance.historyCacheHitRate < 0.3 &&
            ...
            currentConfig.minimumCacheTokens = Math.max(512, currentConfig.minimumCacheTokens * 0.75);
            optimized = true;
        ...
        if (performance.historyCacheHitRate > 0.8 && currentConfig.historySegmentSize > 2) {
            currentConfig.historySegmentSize = Math.max(2, currentConfig.historySegmentSize - 1);
            optimized = true;
...
`;
     ...
     */
    startPerformanceMonitoring(intervalMinutes = 30) {
        ...
        this.performanceMonitoringInterval = setInterval(() => {
            ...
            const performance = this.getCachePerformanceSnapshot();
            if (performance.overallCacheEfficiency < 0.3 &&
     ...
     */
    async warmupSystemMessageCache() {
        if (!this.cacheConfig.enableSystemMessageCaching || !this.isAnthropicModel()) {
            logger.info('System message cache warmup skipped - not enabled or not Anthropic model');
            return;
        }
        try {
    ...
    setActiveConversationWithSync(conversationId, frontendMessages) {
        this.currentConversationId = conversationId;
    ...
    restoreConversationHistory(conversationId, messages) {
    ...
    syncConversationFromFrontend(conversationId, frontendMessages) {
        // Convert frontend message format to AI service format
        const aiMessages = frontendMessages
    ...
    async sendMessage(userMessage, conversationId, modelName) {
        try {
            ...
            const conversationHistory = this.getConversationMessages(this.currentConversationId);
            const cachedHistory = this.prepareCachedMessages(conversationHistory);
            const messages = [systemMessage, ...cachedHistory];
            ...
            const conversationLength = this.getConversationMessages(this.currentConversationId).length;
            const systemCacheUsed = this.cacheConfig.enableSystemMessageCaching && this.isAnthropicModel();
            const historyCacheUsed = this.cacheConfig.enableHistoryCaching &&
                this.isAnthropicModel() &&
            ...
            const estimatedHistoryTokens = historyCacheUsed
                ? this.estimateTokens(cachedHistory
            ...
            const totalTokens = this.estimateTokens(messages
                .map((msg) => typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content))
                .join(' '));
            this.recordCacheMetrics(systemCacheUsed, historyCacheUsed, conversationLength, estimatedTokensSaved, totalTokens, result.providerMetadata);
            // Record usage and cost data
            const usageData = this.recordUsageAndCost(result, this.currentConversationId, this.currentModelName);
            if (usageData) {
    ...
    async sendMessageStream(userMessage, requestId, conversationId, modelName) {
        try {
            // Switch model if specified
            if (modelName) {
                this.switchModel(modelName);
            ...
            try {
                ...
                const totalTokens = this.estimateTokens(messages
                    .map((msg) => typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content))
                    .join(' '));
                this.recordCacheMetrics(systemCacheUsed, historyCacheUsed, conversationLength, estimatedTokensSaved, totalTokens, undefined // Streaming doesn't provide providerMetadata in the same way
                );
