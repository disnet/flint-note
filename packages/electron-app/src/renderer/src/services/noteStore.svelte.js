...
function createNotesStore() {
    ...
    async function loadNoteTypes() {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : 'Failed to load note types';
    ...
    function generateSnippet(content, maxLength = 150) {
    ...
    async function loadNotesOfType(type) {
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : `Failed to load notes for type: ${type}`;
            state.error = errorMessage;
    ...
    async function loadAllNotes() {
        try {
            ...
            const sortedNotes = loadedNotes.sort((a, b) => new Date(b.modified).getTime() - new Date(a.modified).getTime());
    ...
    function handleToolCall(_toolCall) {
