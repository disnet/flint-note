...
export class ElectronChatService {
    constructor() {
    ...
    initializeUsageTracking() {
        ...
        if (window.api?.onUsageRecorded) {
            window.api.onUsageRecorded((usageData) => {
    ...
    sendMessageStream(text, conversationId, onChunk, onComplete, onError, model, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    onToolCall) {
        const requestId = crypto.randomUUID();
        window.api.sendMessageStream({ message: text, conversationId, model, requestId }, (data) => {
            // Stream started
    ...
    async syncConversation(conversationId, messages) {
        try {
    ...
    async setActiveConversation(conversationId, messages) {
        try {
    ...
    async getCachePerformanceSnapshot() {
        try {
    ...
    async startPerformanceMonitoring(intervalMinutes = 30) {
        try {
