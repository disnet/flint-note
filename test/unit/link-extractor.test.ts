...
describe('LinkExtractor', () => {
  ...
  beforeEach(async () => {
    ...
    await db.run(
      ...
      ['general/test-note.md', 'Test Note', 'Content here', 'general', 'test-note.md', '/test/path', '2024-01-01', '2024-01-01']
    );
    await db.run(
      'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      ['projects/my-project.md', 'My Project', 'Project content', 'projects', 'my-project.md', '/test/path2', '2024-01-01', '2024-01-01']
    );
  });
...
Also plain: https://example.com`;
      ...
      const firstOccurrence = result.external_links.find(link => link.url === 'https://example.com');
...
Just some regular content.`;
  ...
  describe('storeLinks', () => {
    test('should store wikilinks and external links in database', async () => {
      const extractionResult = {
        ...
        external_links: [
          { url: 'https://example.com', title: 'Example', line_number: 3, link_type: 'url' as const },
          { url: 'http://test.com/img.png', title: 'Test Image', line_number: 4, link_type: 'image' as const }
        ]
      };
      ...
      await db.run(
        'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['test/source-note.md', 'Source Note', 'Content', 'test', 'source-note.md', '/test/path3', '2024-01-01', '2024-01-01']
      );

      await LinkExtractor.storeLinks('test/source-note.md', extractionResult, db);

      // Check stored wikilinks
      const wikilinks = await db.all('SELECT * FROM note_links WHERE source_note_id = ?', ['test/source-note.md']);
      ...
      const externalLinks = await db.all('SELECT * FROM external_links WHERE note_id = ?', ['test/source-note.md']);
    ...
    test('should replace existing links when storing', async () => {
      ...
      await db.run(
        'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [noteId, 'Test Note', 'Content', 'test', 'note.md', '/test/path', '2024-01-01', '2024-01-01']
      );
      
      // Store initial links
      const firstResult = {
        wikilinks: [{ target_title: 'First Link', line_number: 1 }],
        external_links: [{ url: 'https://first.com', line_number: 1, link_type: 'url' as const }]
      };
      ...
      const secondResult = {
        ...
        external_links: [{ url: 'https://second.com', line_number: 1, link_type: 'url' as const }]
      };
      await LinkExtractor.storeLinks(noteId, secondResult, db);

      // Should only have the new links
      const wikilinks = await db.all('SELECT * FROM note_links WHERE source_note_id = ?', [noteId]);
      const externalLinks = await db.all('SELECT * FROM external_links WHERE note_id = ?', [noteId]);
  ...
  describe('getLinksForNote', () => {
    test('should retrieve all links for a note', async () => {
      ...
      await db.run(
        'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['other/note.md', 'Other Note', 'Content', 'other', 'note.md', '/other/path', '2024-01-01', '2024-01-01']
      );
      ...
      assert.strictEqual(links.outgoing_internal[0].target_note_id, 'general/test-note.md');
  ...
  describe('findBrokenLinks', () => {
    test('should find links with null target_note_id', async () => {
      ...
      await db.run(
        'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['test/note1.md', 'Note 1', 'Content', 'test', 'note1.md', '/test/path1', '2024-01-01', '2024-01-01']
      );
      await db.run(
        'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['test/note2.md', 'Note 2', 'Content', 'test', 'note2.md', '/test/path2', '2024-01-01', '2024-01-01']
      );
      await db.run(
        'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['test/note3.md', 'Note 3', 'Content', 'test', 'note3.md', '/test/path3', '2024-01-01', '2024-01-01']
      );
  ...
  describe('updateBrokenLinks', () => {
    test('should resolve broken links when note is created/renamed', async () => {
      ...
      await db.run(
        'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['new/note.md', 'New Note Title', 'Content', 'new', 'note.md', '/new/path', '2024-01-01', '2024-01-01']
      );

      const updatedCount = await LinkExtractor.updateBrokenLinks('new/note.md', 'New Note Title', db);
  ...
  describe('clearLinksForNote', () => {
    test('should clear all links for a note and set incoming links to broken', async () => {
      ...
      const outgoingInternal = await db.all('SELECT * FROM note_links WHERE source_note_id = ?', [noteId]);
      const outgoingExternal = await db.all('SELECT * FROM external_links WHERE note_id = ?', [noteId]);
