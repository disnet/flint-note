...
describe('Database Schema - Link Tables', () => {
  ...
  beforeEach(async () => {
    ...
    await db.run(
      ...
      ['source/note.md', 'Source Note', 'Content', 'source', 'note.md', '/path1', '2024-01-01', '2024-01-01']
    );
    await db.run(
      'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      ['target/note.md', 'Target Note', 'Content', 'target', 'note.md', '/path2', '2024-01-01', '2024-01-01']
    );
  });
  ...
  describe('note_links table', () => {
    test('should create note_links table with correct schema', async () => {
      ...
      const expectedColumns = ['id', 'source_note_id', 'target_note_id', 'target_title', 'link_text', 'line_number', 'created'];
    ...
    test('should have proper indexes for performance', async () => {
      const indexes = await db.all("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='note_links'");
      ...
      assert.ok(indexNames.includes('idx_note_links_target_title'), 'Should have target title index');
    });
  ...
  describe('external_links table', () => {
    test('should create external_links table with correct schema', async () => {
      ...
      const columnNames = tableInfo.map((col: any) => col.name);
      const expectedColumns = ['id', 'note_id', 'url', 'title', 'line_number', 'link_type', 'created'];
    ...
    test('should have proper indexes for performance', async () => {
      const indexes = await db.all("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='external_links'");
      const indexNames = indexes.map((idx: any) => idx.name);
  ...
  describe('Complex queries on link tables', () => {
    beforeEach(async () => {
      ...
      await db.run(
        'INSERT INTO notes (id, title, content, type, filename, path, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        ['hub/central.md', 'Central Hub', 'Content', 'hub', 'central.md', '/path3', '2024-01-01', '2024-01-01']
      );
