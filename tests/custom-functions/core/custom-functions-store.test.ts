...
describe('CustomFunctionsStore', () => {
  ...
  describe('Basic Operations', () => {
    ...
    it('should create vault directory if it does not exist', async () => {
      ...
      const storagePath = path.join(nonExistentPath, '.flint-note', 'custom-functions.json');
  ...
  describe('CRUD Operations', () => {
    ...
    it('should update existing custom functions', async () => {
      ...
      expect(updated.metadata.updatedAt.getTime()).toBeGreaterThan(created.metadata.createdAt.getTime());
    });
  ...
  describe('Usage Statistics', () => {
    it('should update usage statistics correctly', async () => {
      ...
      expect(updated!.metadata.lastUsed!.getTime()).toBeGreaterThan(testFunc.metadata.createdAt.getTime());
    });
  ...
  describe('Search and Filtering', () => {
    ...
    it('should get functions by tags', async () => {
      ...
      expect(searchTagged.map(f => f.name).sort()).toEqual(['complexSearch', 'searchTest1']);

      const testTagged = await store.getByTags(['test']);
      expect(testTagged).toHaveLength(2);
      expect(testTagged.map(f => f.name).sort()).toEqual(['searchTest1', 'searchTest2']);
    });
    ...
    it('should search functions by name or description', async () => {
      ...
      expect(nameSearch.map(f => f.name).sort()).toEqual(['searchTest1', 'searchTest2']);
  ...
  describe('Backup and Restore', () => {
    ...
    it('should validate function data in backup', async () => {
      ...
      await expect(store.restore(invalidBackup)).rejects.toThrow('Invalid function in backup data');
    });
  });
  ...
  describe('Concurrent Access and Error Handling', () => {
    ...
    it('should handle corrupted storage files gracefully', async () => {
      ...
      const storagePath = path.join(testVaultPath, '.flint-note', 'custom-functions.json');
