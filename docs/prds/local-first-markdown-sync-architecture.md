...
async function deriveEncryptionKey(
  ...
  const didHash = await crypto.subtle.digest(
    'SHA-256',
...
export class EncryptedR2SyncAdapter implements StorageAdapter {
  ...
  async loadRange(keyPrefix: string[]): Promise<Uint8Array[]> {
    ...
    const response = await this.r2.send(new ListObjectsV2Command({
      Bucket: this.bucketName,
      Prefix: prefix
    }));
  ...
  private async updateSyncManifest(documentId: string): Promise<void> {
    ...
    try {
      const existing = await this.r2.send(new GetObjectCommand({
        Bucket: this.bucketName,
        Key: manifestKey
      }));
