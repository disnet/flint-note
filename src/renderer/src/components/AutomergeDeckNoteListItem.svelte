<script lang="ts">
  ...
  interface Props {
    ...
    onTypeChange?: (newTypeId: string) => Promise<void>;
  ...
  // eslint-disable-next-line svelte/prefer-writable-derived -- intentional: local editing state
  ...
  // Handle type change
  async function handleTypeChange(event: Event): Promise<void> {
    const select = event.target as HTMLSelectElement;
    if (onTypeChange) {
      await onTypeChange(select.value);
    }
  }
  ...
  // Get note title by ID from Automerge
  ...
  // Get field type
  function getFieldType(field: string): AutomergeFieldType | 'system' | 'unknown' {
    if (
      field === 'title' ||
      field === 'type' ||
      field === 'flint_title' ||
    ...
    <div class="note-title-row">
      ...
      <div class="type-dropdown-wrapper" onmousedown={(e) => e.stopPropagation()}>
        <select
          ...
          title={note.typeName}
        >
          {#each noteTypes as type (type.id)}
            <option value={type.id}>{type.icon || '📝'} {type.name}</option>
          {/each}
        </select>
        <span class="type-icon-display">{note.typeIcon || '📝'}</span>
      </div>
      <textarea
...
<style>
  ...
  .type-dropdown-wrapper:hover .type-icon-display {
    background: var(--bg-secondary);
    border-color: var(--border-light);
  }

  .type-select:focus + .type-icon-display {
