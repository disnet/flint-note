<script lang="ts">
  import type { WeekData } from '../lib/automerge';
  import {
    formatWeekRange,
    getCurrentWeek,
    isSameWeek,
  ...
  interface Props {
    weekData: WeekData;
    onNavigateToWeek: (startDate: string) => void;
  }

  let { weekData, onNavigateToWeek }: Props = $props();
  ...
  function handlePreviousWeek(): void {
    ...
    onNavigateToWeek(previousWeek.startDate);
  }
  ...
  function handleNextWeek(): void {
    ...
    onNavigateToWeek(nextWeek.startDate);
  }

  function handleTodayClick(): void {
    if (!isCurrentWeek) {
      onNavigateToWeek(currentWeek.startDate);
    }
  }
