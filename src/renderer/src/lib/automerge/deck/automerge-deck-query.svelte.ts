/**
 * Client-side query service for Automerge Deck widgets
...
import { noteToResultNote, EMPTY_FILTER_VALUE } from './automerge-deck-types';

/**
 * Execute a deck query against the provided notes
 * Filters, sorts, and paginates entirely client-side
 */
export function runAutomergeDeckQuery(
  config: DeckConfig,
  allNotes: Note[],
  noteTypes: Record<string, NoteType>,
  options: AutomergeDeckQueryOptions = {}
): AutomergeDeckQueryResult {
...
export function getAvailableFields(
  ...
  // System fields
  const fields: AutomergeFilterFieldInfo[] = [
