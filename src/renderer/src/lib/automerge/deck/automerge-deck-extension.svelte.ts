...
import { automergeDeckTheme } from './automerge-deck-theme';
import {
  parseDeckYamlWithWarnings,
  serializeDeckConfig,
  type DeckValidationWarning
} from '../../../../../shared/deck-yaml-utils';
import type { DeckConfig } from './automerge-deck-types';
import AutomergeDeckWidget from '../../../components/AutomergeDeckWidget.svelte';
import { getNote, getNoteType, updateNote } from '../state.svelte';

/**
 * Handlers for deck events
 */
export interface AutomergeDeckHandlers {
...
const activeWidgets = new Set<
  AutomergeDeckWidgetType | EmbeddedDeckWidgetType | DeckErrorWidgetType
>();

// StateField for storing handlers
const deckHandlersField = StateField.define<AutomergeDeckHandlers | null>({
  create() {
    return null;
  },
  update(handlers) {
...
class EmbeddedDeckWidgetType extends WidgetType {
  ...
  constructor(
    ...
    private handlers: AutomergeDeckHandlers
  ) {
    super();
    this.yamlContent = yamlContent;
  }
  ...
  toDOM(): HTMLElement {
    ...
    // Mount the component
    this.component = mount(AutomergeDeckWidget, {
      target: container,
      props: {
        config: this.config,
 ...
 */
class AutomergeDeckWidgetType extends WidgetType {
  private component: ReturnType<typeof mount> | null = null;
  ...
  eq(other: AutomergeDeckWidgetType): boolean {
    if (skipWidgetComparison) {
      return true;
    }
    return this.yamlContent === other.yamlContent;
 ...
 */
export function automergeDeckExtension(handlers: AutomergeDeckHandlers): Extension {
  return [automergeDeckTheme, deckHandlersField.init(() => handlers), deckField];
}
