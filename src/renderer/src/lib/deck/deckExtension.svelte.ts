 ...
 * This prevents the widget from showing raw content when cursor lands at boundary after config updates
 */
...
class EmbeddedDeckWidgetType extends WidgetType {
  ...
  /**
   * Fetch the deck note and parse its config
   */
  private async loadDeckConfig(): Promise<void> {
    try {
      const note = await window.api?.getNote({ identifier: this.noteId });
      if (!note) {
        this.loadError = `Deck not found: ${this.noteId}`;
        return;
      }

      // Check if note is actually a deck
      if (note.metadata?.flint_kind !== 'deck') {
   ...
   */
  private async saveConfigToSourceNote(newConfig: DeckConfig): Promise<void> {
    try {
  ...
  toDOM(_view: EditorView): HTMLElement {
    ...
    this.loadDeckConfig().then(() => {
      ...
      // Clear loading state and mount the component
      container.innerHTML = '';
...
function decorateDeckBlocks(state: EditorState): DecorationSet {
  ...
  for (const widget of activeWidgets) {
    ...
    if (widget instanceof EmbeddedDeckWidgetType) {
      key = `embed:${(widget as EmbeddedDeckWidgetType).yamlContent.match(EMBED_SYNTAX_REGEX)?.[1]}`;
