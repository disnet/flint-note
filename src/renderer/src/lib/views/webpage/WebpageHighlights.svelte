<script lang="ts">
  import type { WebpageHighlight } from './types';

  let {
    highlights = [],
    onNavigate = (_highlight: WebpageHighlight) => {},
    onDelete = (_id: string) => {},
    onClose = () => {}
  }: {
    highlights: WebpageHighlight[];
    onNavigate?: (highlight: WebpageHighlight) => void;
  ...
  <div class="highlights-content">
    {#if highlights.length === 0}
      <p class="empty-highlights">
        No highlights yet. Select text in the article to highlight it.
      </p>
    {:else}
      <ul class="highlights-list">
        {#each highlights as highlight (highlight.id)}
          <li class="highlight-item">
            <button class="highlight-text" onclick={() => onNavigate(highlight)}>
              "{truncateText(highlight.textContent)}"
            </button>
