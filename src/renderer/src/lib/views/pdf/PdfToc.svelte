<script lang="ts">
  ...
  let {
    outline = [],
    onNavigate = (_dest: string | unknown[]) => {},
    onClose = () => {}
  }: {
  ...
  function handleItemClick(item: PdfOutlineItem): void {
    onNavigate(item.dest);
  }

  function getItemKey(item: PdfOutlineItem, idx: number): string {
...
<div class="pdf-toc">
  ...
  <div class="toc-content">
    ...
    {:else}
      <ul class="toc-list">
        {#each outline as item, idx (getItemKey(item, idx))}
...
{#snippet tocItem(item: PdfOutlineItem, level: number, keyPrefix: string)}
  <li class="toc-item" style="--indent: {level}">
    <div class="toc-item-row">
      {#if item.items && item.items.length > 0}
        <button
          class="expand-button"
          class:expanded={expandedItems.has(item.title)}
          onclick={() => toggleExpand(item.title)}
          aria-label={expandedItems.has(item.title) ? 'Collapse' : 'Expand'}
        >
    ...
    {#if item.items && item.items.length > 0 && expandedItems.has(item.title)}
      <ul class="toc-sublist">
        {#each item.items as subitem, subIdx (`${keyPrefix}-${subIdx}-${subitem.title}`)}
