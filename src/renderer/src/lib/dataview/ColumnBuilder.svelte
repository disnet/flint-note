<script lang="ts">
  import type { ColumnConfig, ColumnDefinition, FilterFieldInfo } from './types';
  import { normalizeColumn, fieldDefToFilterInfo, SYSTEM_COLUMNS } from './types';
  ...
  let { columns, typeName, onColumnsChange }: Props = $props();
  ...
  const availableFields = $derived.by(() => {
    ...
    const systemFieldInfos: FilterFieldInfo[] = SYSTEM_COLUMNS.filter(
      (col) => col.field !== 'title'
    ).map((col) => ({
      name: col.field,
